...
X = np.vstack((data_cannon, data_carrot, data_bat, data_banana, data_nail))
...
y = np.array(["cannon"] * len(data_cannon) +["carrot"] * len(data_carrot) +["bat"] * len(data_bat) +["banana"] * len(data_banana) +["nail"] * len(data_nail)
)
from sklearn.preprocessing import LabelEncoder
...
model = tf.keras.models.Sequential([
    tf.keras.layers.Dense(6767, input_shape=(784, )),
    tf.keras.layers.Dense(4141, activation='relu'),
    tf.keras.layers.Dense(1414, activation="relu"),
